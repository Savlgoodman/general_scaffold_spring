...
- **API 请求日志** — 自动记录所有 API 请求/响应，AOP 切面异步写入
- **登录日志** — 记录登录成功/失败/锁定/禁用
- **操作审计日志** — `@OperationLog` 注解自动记录增删改操作
- **系统异常日志** — 未处理异常自动入库，含堆栈信息
- 所有日志异步写入数据库，不影响接口性能
- API 日志默认保留 30 天，每天凌晨 3 点自动清理
...
app:
  log:
    store-response-body: false    # 是否存储 API 响应体（默认关闭，节省存储空间）
    api-log-retention-days: 30    # API 日志保留天数（默认 30 天）
```

> **注意**：`store-response-body` 默认关闭。如需存储完整响应体用于调试，设为 `true`。响应体可能较大，长期开启会显著增加数据库占用。
